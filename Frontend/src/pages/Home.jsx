...
function Home() {
  return (
    <div className='overflow-hidden bg-orange-50 text-orange-950'>
      ...
      <section id='reviews' className='mx-auto max-w-7xl px-5 py-20 lg:px-8'><div className='flex flex-col justify-between gap-4 sm:flex-row sm:items-end'><div><p className='text-sm font-bold uppercase tracking-[0.2em] text-orange-600'>Kind words</p><h2 className='mt-2 text-4xl font-black tracking-tight'>The reviews are in.</h2></div><div className='flex items-center gap-3'><span className='text-3xl font-black'>4.9</span><div><Stars /><p className='text-xs text-orange-950/50'>2,000+ reviews</p></div></div></div><div className='mt-10 grid gap-5 lg:grid-cols-3'>{reviews.map((review) => <figure key={review.name} className='rounded-3xl border border-orange-200 bg-white p-7'><Stars /><blockquote className='mt-6 text-lg font-bold leading-8 text-orange-950'>“{review.quote}”</blockquote><figcaption className='mt-8 flex items-center gap-3'><span className='flex h-10 w-10 items-center justify-center rounded-full bg-orange-200 text-xs font-black text-orange-800'>{review.initials}</span><span><strong className='block text-sm'>{review.name}</strong><span className='text-xs text-orange-950/50'>{review.detail}</span></span></figcaption></figure>)}</div></section>

      <section id='feedback' className='border-y border-orange-200 bg-orange-100'><div className='mx-auto grid max-w-7xl gap-8 px-5 py-16 lg:grid-cols-[1fr_auto] lg:items-center lg:px-8'><div><p className='text-sm font-bold uppercase tracking-[0.2em] text-orange-600'>Your voice matters</p><h2 className='mt-2 text-3xl font-black tracking-tight sm:text-4xl'>How did we do?</h2><p className='mt-3 max-w-xl text-sm leading-6 text-orange-950/65'>Tell us about your Bhoj experience. Every rating helps us make the next order even better.</p></div><a href='#feedback-form' className='w-fit rounded-full border-2 border-orange-600 px-7 py-3.5 text-sm font-bold text-orange-700 hover:bg-orange-600 hover:text-white'>Leave feedback <span aria-hidden='true'>↗</span></a></div></section>
